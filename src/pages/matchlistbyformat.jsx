...
function MatchList() {
  ...
  useEffect(() => {
    ...
    const fetchMatches = async () => {
      try {
        const response = await axios.get(`${ BASE_URL }matches/${format}`);
    ...
    <Layout>
          ...
          <div className="w-full min-h-screen  p-4 bg-gray-300 text-center flex justify-center">
            ...
            <table className="md:w-[1000px] bg-white border border-gray-300 rounded-xl shadow-2xl text-center h-fit text-[12px] md:text-lg">
              <thead>
                <tr>
                  <th className="py-2 px-2 border-b text-blue-600">Match</th>
                  <th className="py-2 px-2 border-b text-blue-600">Date</th>
                  {/* <th className="py-2 px-4 border-b text-blue-500">Teams</th> */}
                  {/* <th className="py-2 px-2 border-b text-blue-500">Format</th> */}
                  <th className="py-2 px-2 border-b text-blue-500">Status</th>
                </tr>
              </thead>
              <tbody className='text-center'>
              {matches?.map((match, index) => (
                <tr className='py-10 md:mx-5' key={index}>
                  <td className="py-2 px-2 border-b text-blue-700"><Link to={`matchsections/${match.key}/${match.format}/${match.teams.a.name}/${match.teams.b.name}/bidding`}>{match.name}</Link></td>
                  ...
                  <td className="py-2 px-2 border-b text-gray-800">
                    ...
                    {match.status == "live" && <Link to={`matchsections/${match.key}/${match.format}/${match.teams.a.name}/${match.teams.b.name}/bidding`}> InPlay</Link>}
